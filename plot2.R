plot2 <- function(DataDir, PlotDir=DataDir, 
                    width=480, height=480, DataCache.fg=F, SavePNG.fg=T,
                    Variable="Global_active_power", main="Global Active Power", 
...
#(RoughMemorySize <- 2075259*9*8/(2^20))
    ...
    if(SavePNG.fg) png(file="plot2.png", width=width, height=height, units="px")
    plot(DateTime.vt, data.df[,Variable], type="l", axes=T, main = main, ylab = ylab, xlab = xlab)
